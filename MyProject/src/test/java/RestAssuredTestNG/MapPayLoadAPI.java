...
public class MapPayLoadAPI {
	...
	public void TrelloPost() {
		...
		mp.put("name", "18042024");
		 Response res=given(). queryParam("token", token).queryParam("key", key)
		...
		//put operation 
		try {
			given().queryParam("name", updateName)
			.queryParam("key", key)
			.queryParam("token", token)
			.queryParam("prefs/background", updateColor)
			.header("Content-Type","application/json").pathParam("id", id).log().all()
			.when().put("/boards/{id}")			
			 .then().log().all().assertThat().statusCode(200) .body("name",
			  Matchers.equalTo(updateName))
			  .body("prefs.background",
					  Matchers.equalTo(updateColor)).statusCode(200);
		...
		//get operation
		  ...
		  given() .queryParam("key", key) .queryParam("token", token)
		  .header("Content-Type","application/json").pathParam("id", id).log().all()
		  .when().delete("/boards/{id}").then().log().all().assertThat().statusCode(200);
